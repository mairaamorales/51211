// Generated from c:/Users/Netbook/antlr/proyecto/Lenguaje.g4 by ANTLR 4.13.2
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete listener for a parse tree produced by LenguajeParser.
export default class LenguajeListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by LenguajeParser#programa.
	enterPrograma(ctx) {
	}

	// Exit a parse tree produced by LenguajeParser#programa.
	exitPrograma(ctx) {
	}


	// Enter a parse tree produced by LenguajeParser#instruccion.
	enterInstruccion(ctx) {
	}

	// Exit a parse tree produced by LenguajeParser#instruccion.
	exitInstruccion(ctx) {
	}


	// Enter a parse tree produced by LenguajeParser#asignacion.
	enterAsignacion(ctx) {
	}

	// Exit a parse tree produced by LenguajeParser#asignacion.
	exitAsignacion(ctx) {
	}


	// Enter a parse tree produced by LenguajeParser#excepcion.
	enterExcepcion(ctx) {
	}

	// Exit a parse tree produced by LenguajeParser#excepcion.
	exitExcepcion(ctx) {
	}


	// Enter a parse tree produced by LenguajeParser#bloque.
	enterBloque(ctx) {
	}

	// Exit a parse tree produced by LenguajeParser#bloque.
	exitBloque(ctx) {
	}


	// Enter a parse tree produced by LenguajeParser#imprimir.
	enterImprimir(ctx) {
	}

	// Exit a parse tree produced by LenguajeParser#imprimir.
	exitImprimir(ctx) {
	}


	// Enter a parse tree produced by LenguajeParser#expresion.
	enterExpresion(ctx) {
	}

	// Exit a parse tree produced by LenguajeParser#expresion.
	exitExpresion(ctx) {
	}


	// Enter a parse tree produced by LenguajeParser#termino.
	enterTermino(ctx) {
	}

	// Exit a parse tree produced by LenguajeParser#termino.
	exitTermino(ctx) {
	}


	// Enter a parse tree produced by LenguajeParser#numero.
	enterNumero(ctx) {
	}

	// Exit a parse tree produced by LenguajeParser#numero.
	exitNumero(ctx) {
	}


	// Enter a parse tree produced by LenguajeParser#identificador.
	enterIdentificador(ctx) {
	}

	// Exit a parse tree produced by LenguajeParser#identificador.
	exitIdentificador(ctx) {
	}


	// Enter a parse tree produced by LenguajeParser#op.
	enterOp(ctx) {
	}

	// Exit a parse tree produced by LenguajeParser#op.
	exitOp(ctx) {
	}



}